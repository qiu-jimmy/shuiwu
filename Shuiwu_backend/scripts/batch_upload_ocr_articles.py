...
# Articles 文件夹路径
ARTICLES_DIR = r"C:\Users\yan\Downloads\实务指南-33个税收政策指引2025\案例判例-实务案例260130止"

# 目标知识库表名
TARGET_TABLE_NAME = "kb_user_268_3480c9e3"
...
def process_single_file(file_path: Path, ocr_processor: PaddleOCRProcessor, kb, table_name: str, stats: Dict, lock: threading.Lock, index: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            # 更新 document_ids（只在新上传文件时添加，避免重复）
            # 如果复用已有记录且文档已存在于知识库，不需要重复添加
            should_add_to_registry = False
            if not existing_file_record:
                # 新上传的文件，需要添加到 registry
                should_add_to_registry = True
            else:
                # 复用已有记录，检查是否真的在知识库表中
                # 如果不在，说明之前上传失败，需要重新添加到 registry
                if not knowledge_service.repository.check_document_exists(table_name, filename):
                    should_add_to_registry = True

            if should_add_to_registry:
                knowledge_service._add_document_to_registry(table_name, file_id_to_use, filename)
                print(f"        [INFO] 已添加到 document_ids: file_id={file_id_to_use}")
