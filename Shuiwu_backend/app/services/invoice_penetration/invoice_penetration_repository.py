...
logger = get_logger("app.services.invoice_penetration.repository")


class InvoicePenetrationRepository:
    """发票穿透报告数据访问层"""

    def __init__(self, db_schema: str = "business"):
        self.db_schema = db_schema
        self.table = f"{db_schema}.invoice_penetration_reports"
    ...
    def create_report(self, user_id: str, taxpayer_no: str, company_name: str, order_no: str) -> Optional[int]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                logger.info(f"创建发票穿透报告记录成功: id={report_id}, order_no={order_no}")
                return report_id
        except Exception as e:
            logger.error(f"创建发票穿透报告记录失败: {e}")
            return None
    ...
    def get_report_by_order_no(self, order_no: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"根据订单号获取报告记录失败: {e}")
            return None
    ...
    def update_report_status(
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                """), params)
                conn.commit()
                logger.info(f"更新报告状态成功: order_no={order_no}, status={status}")
                return True
        except Exception as e:
    ...
    def list_reports_by_user(
        ...
        try:
            ...
            logger.error(f"获取用户发票穿透报告列表失败: {e}")
            return {"total": 0, "page": page, "page_size": page_size, "reports": []}


# 全局实例
invoice_penetration_repository = InvoicePenetrationRepository()
